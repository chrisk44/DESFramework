...
int ParallelFramework::run() {
	...
	*/
	...
	bool* results = (bool*)malloc(sizeof(bool) * N[parameters->D - 1]);
...
void ParallelFramework::scanDimension(int d, float* point, bool* results, int startIdx) {
	// Go through every point in dimension d, using the pre-set values in 'point' for the smaller dimensions
	...
	if (d == 0) {
		...
		for (int i = 0; i < N[d]; i++) {
			...
			results[startIdx + i] = model->validate_cpu(point);
		}
		...
		for (int i = 0; i < N[d]; i++) {
			...
			int tmpStartIdx = startIdx + i * idxSteps[d];
...
bool ParallelFramework::getResultAt(float* point) {
	...
	for (int i = 0; i < parameters->D; i++) {
		dimSteps = (int) floor(abs(limits[i].lowerLimit - point[i]) * abs(limits[i].upperLimit - limits[i].lowerLimit) / N[i]);
		index += dimSteps;
	}
